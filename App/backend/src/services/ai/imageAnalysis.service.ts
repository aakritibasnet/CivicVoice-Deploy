...
export type ImageAnalysisResult = {
  ...
  title: string;
  description: string;
  suggested_priority: ReportPriorityLevel;
};
...
function coerceCategory(value: unknown): ImageAnalysisResult["category"] {
...
function coercePriority(value: unknown): ReportPriorityLevel {
  const v = String(value ?? "").trim().toLowerCase();
  return (PRIORITY_LEVELS as readonly string[]).includes(v)
    ? (v as ReportPriorityLevel)
...
export async function analyzeReportImage(
  ...
  try {
    ...
    const reason =
      err?.name === "AbortError" ? "timed out" : "could not be reached";
  ...
  if (!res.ok) {
    ...
    console.error(
      `Gemini analyze-image failed: ${res.status} ${res.statusText} ${detail.slice(0, 500)}`,
    );
