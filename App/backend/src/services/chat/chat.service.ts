...
type CreateChatInput = {
  ...
  isGroup?: boolean;
  /** Extra participants beyond the creator: [{ kind, id, role? }]. */
  participants?: {
    kind: "user" | "officer";
    id: string;
...
export async function createChat(
  ...
  let complaintWardId: string | null = null;
  ...
  const municipalityId =
    input.municipalityId ?? principal.municipalityId ?? null;
    ...
    if (existing.rows[0]) {
      ...
      await pool.query(
        `INSERT INTO chat_participants
         ...
         ON CONFLICT (chat_id, party_kind, party_id) DO NOTHING`,
        [existing.rows[0].id, principal.kind, principal.id],
      );
    ...
    const existing = await pool.query(
      `SELECT ${CHAT_RETURN_COLS}
        ...
        WHERE type = $5::chat_type
          ...
          AND is_group = false
        LIMIT 1`,
      [principal.kind, principal.id, other.kind, other.id, input.type],
    );
    if (existing.rows[0]) {
      await pool.query(
        `INSERT INTO chat_participants
  ...
  try {
        ...
        if (winner.rows[0]) {
          await pool.query(
            `INSERT INTO chat_participants
             ...
             VALUES ($1, $2, $3, 'member'::chat_role)
             ON CONFLICT (chat_id, party_kind, party_id) DO NOTHING`,
            [winner.rows[0].id, principal.kind, principal.id],
          );
    ...
    const rows: { kind: string; id: string; role: string }[] = [
      { kind: principal.kind, id: principal.id, role: "admin" },
    ];
    for (const p of input.participants ?? []) {
      if (p.kind === principal.kind && p.id === principal.id) continue;
...
export async function listMyChats(principal: ResolvedPrincipal) {
  ...
  const { rows } = await pool.query(
    ...
    [principal.kind, principal.id, principal.wardId, municipalityScopeId],
  );
  return rows;
}
...
export async function getParticipants(
  ...
  const { rows } = await pool.query(
    `SELECT party_kind, party_id, role_in_chat::text AS role_in_chat,
