...
// "write"    = post a message (blocked once resolved/closed)
// "workflow"  = civic actions (escalate/status/acknowledge): write-level
//               membership, but allowed on resolved/closed so a chat can be
//               reopened and a closed thread can still be acknowledged
// "manage"   = chat admin only (rename/participants — Sprint 8)
export type ChatAction = "read" | "write" | "workflow" | "manage";
...
export type AccessDecision = {
  allowed: boolean;
  /** Machine-readable reason — asserted on directly by the matrix tests. */
  reason: string;
};
...
export function evaluateChatAccess(
  ...
  const wardMatch =
    !!chat.ward_id && principal.wardId === chat.ward_id;
  const muniMatch =
...
export async function assertChatAccess(
  ...
  const chat = chatRes.rows[0] as ChatRecord | undefined;
  if (!chat) {
    throw new AppError("Chat not found", 404);
  }

  const partRes = await pool.query(
    `SELECT role_in_chat::text AS role_in_chat, is_active
       FROM chat_participants
      WHERE chat_id = $1 AND party_kind = $2 AND party_id = $3`,
    [chatId, principal.kind, principal.id],
  );
  const participant = (partRes.rows[0] as ParticipantRecord | undefined) ?? null;
