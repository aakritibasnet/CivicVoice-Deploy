...
const ALLOWLIST: Record<string, ResourceType> = {
  "image/jpeg": "image",
  "image/png": "image",
  "image/gif": "image",
  "image/webp": "image",
...
export async function uploadChatAttachment(
  ...
  try {
    ...
    await client.query(
      `UPDATE chats SET last_message_at = $2,
...
export async function getAttachmentForDownload(
  ...
  const { rows } = await pool.query(
    `SELECT a.id, a.file_name, a.mime_type, a.storage_key, a.thumbnail_key,
  ...
  try {
    ...
    await writeAudit(pool, {
      ...
      metadata: { attachmentId, reason: (err as AppError)?.message },
    }).catch(() => {});
    throw err;
  }
