...
export type CreateAnnouncementInput = {
  ...
  /** Ward ID for all_ward_officers / ward_specific; department ID for department. */
  scopeId?: string | null;
  body: string;
};
...
): Promise<{ id: string; kind: "officer" | "user" }[]> {
  switch (input.scope) {
    case "all_municipality_officers": {
      const muniId = principal.municipalityId;
      if (!muniId) throw new AppError("Sender has no municipality context", 403);
      const { rows } = await pool.query(
        `SELECT id FROM officers WHERE municipality_id = $1`,
        [muniId],
      );
      return rows.map((r: { id: string }) => ({ id: r.id, kind: "officer" as const }));
    }

    case "all_ward_officers": {
      const wardId = input.scopeId ?? principal.wardId;
      if (!wardId) throw new AppError("Ward ID required for all_ward_officers scope", 400);
      const { rows } = await pool.query(
        `SELECT id FROM officers WHERE ward_id = $1`,
        [wardId],
      );
      return rows.map((r: { id: string }) => ({ id: r.id, kind: "officer" as const }));
    }
    ...
    case "department": {
      if (!input.scopeId) throw new AppError("scopeId (department) required", 400);
      const { rows } = await pool.query(
        `SELECT id FROM officers WHERE department_id = $1`,
        [input.scopeId],
      );
      return rows.map((r: { id: string }) => ({ id: r.id, kind: "officer" as const }));
    }
    ...
    case "ward_specific": {
      const wardId = input.scopeId ?? principal.wardId;
      if (!wardId) throw new AppError("Ward ID required for ward_specific scope", 400);
      // Active participants across all chats in this ward, deduplicated.
      const { rows } = await pool.query(
        `SELECT DISTINCT cp.party_kind AS kind, cp.party_id AS id
           FROM chat_participants cp
           JOIN chats c ON c.id = cp.chat_id
          WHERE c.ward_id = $1 AND cp.is_active = TRUE`,
        [wardId],
      );
      return rows.map((r: { kind: string; id: string }) => ({
        id: r.id,
        kind: r.kind as "officer" | "user",
      }));
    }
...
export async function sendAnnouncement(
  ...
  if (!input.body?.trim()) throw new AppError("Body required", 400);
...
export async function listAnnouncements(
  ...
  const { rows } = await pool.query(
    `SELECT id, sender_kind, sender_id, scope, scope_id, body, sent_count, created_at
       FROM chat_announcements
      WHERE municipality_id = $1
        AND ($2::timestamptz IS NULL OR created_at < $2::timestamptz)
      ORDER BY created_at DESC
      LIMIT $3`,
    [principal.municipalityId ?? null, before ?? null, Math.min(100, limit)],
  );
  return rows;
}
