...
function principal(p: Partial<ResolvedPrincipal>): ResolvedPrincipal {
  return {
    kind: "officer",
    id: "p-1",
    role: "officer",
...
function chat(p: Partial<ChatRecord>): ChatRecord {
  return {
    id: "c-1",
    type: "officer_ward",
    status: "open",
...
const member: ParticipantRecord = { role_in_chat: "member", is_active: true };
const admin: ParticipantRecord = { role_in_chat: "admin", is_active: true };
const viewer: ParticipantRecord = { role_in_chat: "viewer", is_active: true };
...
test("inactive participant is denied outright", () => {
  const pr = principal({ wardId: WARD_A });
  const c = chat({ type: "officer_ward", ward_id: WARD_A });
  const d = evaluateChatAccess(
    pr,
    c,
    { role_in_chat: "member", is_active: false },
    "read",
  );
  assert.equal(d.allowed, false);
...
test("manage requires an admin participant row", () => {
  ...
  const c = chat({ type: "officer_ward", ward_id: WARD_A });
  assert.equal(
    evaluateChatAccess(pr, c, member, "manage").reason,
  ...
  assert.equal(
    evaluateChatAccess(pr, c, null, "manage").reason,
    "manage_requires_admin",
  );
});
...
test("viewers still cannot perform workflow actions", () => {
  ...
  assert.equal(
    evaluateChatAccess(pr, c, viewer, "workflow").reason,
    "viewer_read_only",
  );
});
