...
export type ResolvedPrincipal = {
  kind: PrincipalKind;
  id: string;
  /** `users.role` for users; always `"officer"` for the officers table. */
  role: string;
  officerType: "ward_officer" | "municipality_officer" | null;
  wardId: string | null;
  municipalityId: string | null;
  departmentId: string | null;
};

async function loadUser(id: string): Promise<ResolvedPrincipal | null> {
...
async function loadOfficer(id: string): Promise<ResolvedPrincipal | null> {
  const { rows } = await pool.query(
    `SELECT o.id,
...
async function inferMunicipalityScopeForOfficer(): Promise<string | null> {
  const { rows } = await pool.query<{ municipality_id: string | null }>(
    `SELECT w.municipality_id
      ...
      LIMIT $1`,
    [MUNICIPALITY_OFFICER_SCOPE_FALLBACK_LIMIT],
  );
