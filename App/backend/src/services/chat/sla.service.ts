...
export async function setSlaDeadline(
  ...
  chatId: string,
  resolveByHours: number,
  escalateByHours?: number | null,
) {
  ...
  try {
    ...
    // Upsert the config.
    await client.query(
      `INSERT INTO chat_sla_configs (chat_id, resolve_by_hours, escalate_by_hours)
       VALUES ($1, $2, $3)
       ON CONFLICT (chat_id) DO UPDATE
         SET resolve_by_hours  = EXCLUDED.resolve_by_hours,
             escalate_by_hours = EXCLUDED.escalate_by_hours,
             updated_at        = CURRENT_TIMESTAMP`,
      [chatId, resolveByHours, escalateByHours ?? resolveByHours * 2],
    );
    ...
    const timerRes = await client.query(
      `INSERT INTO chat_sla_timers
       ...
       RETURNING id, resolve_deadline, escalate_deadline`,
      [chatId, resolveByHours, escalateByHours ?? null],
    );
...
export async function resolveSlaTimer(chatId: string): Promise<void> {
  await pool.query(
    `UPDATE chat_sla_timers
        SET resolved_at = CURRENT_TIMESTAMP
      WHERE chat_id = $1 AND resolved_at IS NULL`,
...
export async function getActiveSlaTimer(chatId: string) {
  const { rows } = await pool.query(
    `SELECT t.id, t.resolve_deadline, t.escalate_deadline,
...
async function runOverdueCheck(): Promise<void> {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT t.id AS timer_id, t.chat_id,
...
async function fireOverdue(timer: {
  ...
  escalate_deadline: Date | null;
}): Promise<void> {
  // Mark fired atomically — if two instances race, only one writes (CAS).
  const { rowCount } = await pool.query(
    `UPDATE chat_sla_timers
        SET overdue_fired_at = CURRENT_TIMESTAMP
      WHERE id = $1 AND overdue_fired_at IS NULL`,
    [timer.timer_id],
  );
  if (!rowCount) return; // Another instance beat us.
  ...
  await Promise.allSettled(
    participants.map((p: { party_kind: string; party_id: string }) =>
      createNotification({
        userId: p.party_id,
