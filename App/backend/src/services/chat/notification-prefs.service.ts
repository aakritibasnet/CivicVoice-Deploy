...
export async function getChatPrefs(
  principal: Pick<ResolvedPrincipal, "kind" | "id">,
): Promise<ChatNotificationPrefs> {
  const { rows } = await pool.query(
    `SELECT push_enabled, in_app_enabled, dnd,
            quiet_hours_start, quiet_hours_end, mention_override
       FROM chat_notification_prefs
      WHERE party_kind = $1 AND party_id = $2`,
...
export async function updateChatPrefs(
  principal: Pick<ResolvedPrincipal, "kind" | "id">,
  patch: Partial<ChatNotificationPrefs>,
): Promise<ChatNotificationPrefs> {
  ...
  await pool.query(
    `INSERT INTO chat_notification_prefs
