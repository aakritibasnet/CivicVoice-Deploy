...
type Recipient = {
  kind: "user" | "officer";
  id: string;
  // Evaluated in SQL so 'infinity' (forever-mute) compares correctly —
  // new Date('infinity') is NaN and would silently read as "not muted".
  is_muted: boolean;
...
type PendingPush = {
  ...
  chat: ChatMeta;
  recipient: Recipient;
  // Most-recent action context (preview + verb) the recipient hasn't seen yet.
  lastBody: string;
  lastVerb: "messaged" | "replied";
  senderLabel: string;
};

const pending = new Map<string, PendingPush>();

function key(chatId: string, r: Recipient): string {
  return `${chatId}|${r.kind}:${r.id}`;
}

function chatLabel(chat: ChatMeta): string {
...
async function resolveSenderLabel(
  ...
  if (senderKind === "officer") {
    ...
    const o = rows[0];
    const personal =
      o && `${o.first_name ?? ""} ${o.last_name ?? ""}`.trim();
  ...
  // sender_kind === "user"
  const { rows } = await pool.query(
    `SELECT u.name, u.ward_id::text AS ward_id, u.municipality_id::text AS municipality_id
...
function buildBodyPreview(message: {
  ...
  if (message.type === "text") return message.body?.slice(0, 140) ?? "";
...
function chatDashboardLink(chatId: string): string {
...
function asPrincipal(r: Recipient): ResolvedPrincipal {
  return {
    kind: r.kind,
    id: r.id,
...
export async function dispatchMessageNotifications(message: {
  ...
  const chatRes = await pool.query(
    `SELECT c.id, c.title, c.type::text AS type,
  ...
  const chat = chatRes.rows[0] as ChatMeta | undefined;
  if (!chat) return;

  const partRes = await pool.query(
    `SELECT party_kind AS kind, party_id AS id, notification_level,
            (muted_until IS NOT NULL
             AND muted_until > CURRENT_TIMESTAMP) AS is_muted
       FROM chat_participants
      WHERE chat_id = $1 AND is_active = TRUE
        AND NOT (party_kind = $2 AND party_id = $3)`,
    [message.chat_id, message.sender_kind, message.sender_id],
  );
  ...
  const priorByOtherRes = await pool.query(
    `SELECT sender_kind, sender_id::text AS sender_id
       FROM messages
      WHERE chat_id = $1 AND id <> $2::uuid AND deleted_at IS NULL
        AND NOT (sender_kind = $3 AND sender_id = $4)`,
    [message.chat_id, message.id, message.sender_kind, message.sender_id],
  );
  const prevSpeakers = new Set(
    priorByOtherRes.rows.map(
      (r: { sender_kind: string; sender_id: string }) =>
        `${r.sender_kind}:${r.sender_id}`,
    ),
  );

  const mentionSet = new Set(
    (message.mentions ?? []).map((m) => `${m.kind}:${m.id}`),
  );
  const isHighPriority =
    message.priority === "urgent" || message.priority === "emergency";
  const urgentBypassPolicy =
    isHighPriority && (await urgentBypassesMute());

  for (const row of partRes.rows as Recipient[]) {
    ...
    const inRoom = await isPrincipalInChatRoom(
      row.kind,
      row.id,
      message.chat_id,
    );
    if (inRoom) continue; // actively viewing — no push

    const prefs = await getChatPrefs(row);
    const mentioned = mentionSet.has(`${row.kind}:${row.id}`);
    const bypass =
      urgentBypassPolicy || (mentioned && prefs.mention_override);

    const recipientHasSpoken = prevSpeakers.has(`${row.kind}:${row.id}`);
    const verb: "messaged" | "replied" =
      message.reply_to_message_id || recipientHasSpoken
        ? "replied"
        : "messaged";

    if (!bypass) {
      if (row.is_muted) continue;
      if (row.notification_level === "none") continue;
      if (row.notification_level === "mentions" && !mentioned) continue;
      if (!prefs.push_enabled) continue;
      if (prefs.dnd) continue;
      if (isQuietHour(prefs, nowHour)) continue;
    }
...
let urgentBypassCache: { value: boolean; at: number } | null = null;

async function urgentBypassesMute(): Promise<boolean> {
  ...
  const { rows } = await pool.query(
    `SELECT value FROM system_settings WHERE key = 'chat.urgent_bypasses_mute'`,
  );
