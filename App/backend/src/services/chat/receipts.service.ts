...
export async function recordDelivered(
  ...
  const { rowCount } = await pool.query(
    ...
    [chatId, principal.kind, principal.id, messageIds],
  );
  return rowCount ?? 0;
}
...
export async function recordRead(
  ...
  chatId: string,
  uptoMessageId: string | null,
): Promise<{ unread: number; lastReadMessageId: string | null }> {
  await assertChatAccess(principal, chatId, "read");
  ...
  try {
    ...
    await client.query(
      ...
      [chatId, principal.kind, principal.id, marker.id],
    );
...
export async function getUnreadSummary(
  principal: ResolvedPrincipal,
): Promise<{ total: number; perChat: { chatId: string; unread: number }[] }> {
  const { rows } = await pool.query(
    `SELECT cp.chat_id,
...
export function assertHasMessageIds(ids: unknown): string[] {
