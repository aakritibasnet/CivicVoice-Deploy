...
const EDIT_WINDOW_MS =
  Number(process.env.CHAT_EDIT_WINDOW_MIN || 15) * 60 * 1000;

async function loadOwnMessage(chatId: string, messageId: string) {
  const { rows } = await pool.query(
    `SELECT id, sender_kind, sender_id, created_at, deleted_at
...
export async function addReaction(
  ...
  if (!emoji || emoji.length > 16) throw new AppError("Invalid emoji", 400);
  await assertChatAccess(principal, chatId, "read");
  await pool.query(
    `INSERT INTO message_reactions (message_id, party_kind, party_id, emoji)
     SELECT $1::uuid, $2::text, $3::uuid, $4
      WHERE EXISTS (SELECT 1 FROM messages WHERE id = $1 AND chat_id = $5)
     ON CONFLICT (message_id, party_kind, party_id, emoji) DO NOTHING`,
    [messageId, principal.kind, principal.id, emoji, chatId],
  );
  ...
  await pool.query(
    `DELETE FROM message_reactions
      WHERE message_id = $1 AND party_kind = $2 AND party_id = $3
        AND emoji = $4`,
    [messageId, principal.kind, principal.id, emoji],
  );
...
export async function editMessage(
  ...
  if (!body?.trim()) throw new AppError("Body required", 400);
  ...
  if (msg.deleted_at) throw new AppError("Message deleted", 410);
  if (Date.now() - new Date(msg.created_at).getTime() > EDIT_WINDOW_MS) {
    throw new AppError("Edit window has passed", 403);
  }
  const { rows } = await pool.query(
    `UPDATE messages SET body = $2, edited_at = CURRENT_TIMESTAMP
      WHERE id = $1 RETURNING id, body, edited_at`,
    [messageId, body.trim()],
  );
...
export async function searchMessages(
  ...
  if (!q?.trim()) throw new AppError("Query required", 400);
  await assertChatAccess(principal, chatId, "read");
  const { rows } = await pool.query(
    `SELECT id, sender_kind, sender_id, body, created_at
       FROM messages
      WHERE chat_id = $1 AND deleted_at IS NULL
        AND body ILIKE '%' || $2 || '%'
      ORDER BY created_at DESC
      LIMIT $3`,
    [chatId, q.trim(), Math.min(100, Math.max(1, limit))],
  );
  return rows;
}
