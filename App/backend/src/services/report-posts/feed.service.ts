...
export type FeedSort = "latest" | "top_rated";
...
export async function getReportPostsFeed(params: FeedParams) {
  ...
  if (cursor) {
    if (sort === "top_rated") {
      ...
      conditions.push(
        `(rp.rating_average, rp.created_at) < ($${idx}, $${idx + 1})`,
      );
  ...
  let viewerRatings = new Map<string, number>();
  let viewerBookmarks = new Set<string>();

  if (viewerId && nodes.length > 0) {
    const postIds = nodes.map((n: any) => n.id);

    const [ratingsRes, bookmarksRes] = await Promise.all([
      pool.query(
        `SELECT post_id, rating FROM report_ratings WHERE user_id = $1 AND post_id = ANY($2)`,
        [viewerId, postIds],
      ),
      pool.query(
        `SELECT post_id FROM report_post_bookmarks WHERE user_id = $1 AND post_id = ANY($2)`,
        [viewerId, postIds],
      ),
    ]);
...
export async function getReportPostDetail(postId: string, viewerId?: string | null) {
  const q = `
    SELECT
      rp.id,
      rp.task_id,
  ...
  if (viewerId) {
    const [ratingRes, bookmarkRes] = await Promise.all([
      pool.query(
        `SELECT rating FROM report_ratings WHERE user_id = $1 AND post_id = $2`,
        [viewerId, postId],
      ),
      pool.query(
        `SELECT 1 FROM report_post_bookmarks WHERE user_id = $1 AND post_id = $2`,
        [viewerId, postId],
      ),
    ]);
...
export async function ratePost(
  ...
  // Upsert rating
  await pool.query(
    `INSERT INTO report_ratings (post_id, user_id, rating)
     VALUES ($1, $2, $3)
  ...
  const { rows } = await pool.query(
    `SELECT COALESCE(AVG(rating), 0) AS avg, COUNT(*)::int AS cnt
     FROM report_ratings WHERE post_id = $1`,
    [postId],
  );

  await pool.query(
    `UPDATE report_posts SET rating_average = $1, rating_count = $2, updated_at = NOW()
     WHERE id = $3`,
    [rows[0].avg, rows[0].cnt, postId],
  );
...
export async function togglePostBookmark(postId: string, userId: string) {
  const existing = await pool.query(
    `SELECT id FROM report_post_bookmarks WHERE post_id = $1 AND user_id = $2`,
    [postId, userId],
  );

  if (existing.rows.length > 0) {
    await pool.query(
      `DELETE FROM report_post_bookmarks WHERE post_id = $1 AND user_id = $2`,
      [postId, userId],
    );
    await pool.query(
      `UPDATE report_posts SET bookmark_count = GREATEST(0, bookmark_count - 1) WHERE id = $1`,
      [postId],
    );
  ...
  await pool.query(
    `INSERT INTO report_post_bookmarks (post_id, user_id) VALUES ($1, $2)`,
    [postId, userId],
  );
  await pool.query(
    `UPDATE report_posts SET bookmark_count = bookmark_count + 1 WHERE id = $1`,
    [postId],
  );
...
export async function addPostComment(
  ...
  const { rows } = await pool.query(
    `INSERT INTO report_comments (post_id, user_id, content, anonymous_name)
     VALUES ($1, $2, $3, $4)
     RETURNING id, content, anonymous_name, created_at`,
    [postId, userId, content, userName.split(" ")[0] + " " + String.fromCharCode(65 + Math.floor(Math.random() * 26)) + "."],
  );

  await pool.query(
    `UPDATE report_posts SET comment_count = comment_count + 1 WHERE id = $1`,
    [postId],
  );
