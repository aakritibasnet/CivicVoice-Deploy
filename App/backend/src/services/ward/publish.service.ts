...
type PublishPreview = {
  ...
  changes_since_last: {
    new_tasks: number;
    status_changes: { report_id: string; title: string; old_status: string; new_status: string }[];
    total_changes: number;
  };
  summary: string;
};

type WardPublishingSchemaStatus = {
  hasPublishedReports: boolean;
  hasPublishSchedule: boolean;
};

async function getWardPublishingSchemaStatus(): Promise<WardPublishingSchemaStatus> {
...
async function ensureWardPublishingSchema() {
  ...
  if (!status.hasPublishedReports || !status.hasPublishSchedule) {
    throw new AppError(
      "Ward publishing is not configured yet. Please run the publishing database migration.",
      503,
    );
  }
...
async function getWardTaskSnapshot(wardId: string): Promise<TaskSnapshot[]> {
  const { rows } = await pool.query(
    `SELECT r.id AS report_id, r.title, r.category, r.status,
...
async function getLastPublished(wardId: string) {
  const { rows } = await pool.query(
    `SELECT id, report_snapshot, published_at
...
function detectChanges(
  ...
  previousSnapshot: TaskSnapshot[] | null,
) {
  ...
  const statusChanges: { report_id: string; title: string; old_status: string; new_status: string }[] = [];
...
function generateSummary(
  ...
  if (closed > 0) lines.push(`Closed: ${closed} task${closed !== 1 ? "s" : ""} marked invalid`);
...
export async function getPublishStatus(wardId: string) {
  await ensureWardPublishingSchema();

  const scheduleRes = await pool.query(
    `SELECT last_published_at, next_auto_publish_at, cycle_days
...
export async function getPublishPreview(wardId: string): Promise<PublishPreview> {
  await ensureWardPublishingSchema();
  ...
  const previousSnapshot = lastPublished?.report_snapshot as TaskSnapshot[] | null;
...
export async function publishWardReport(wardId: string, publishedBy: string) {
  await ensureWardPublishingSchema();
  ...
  if (!preview.can_publish) {
    throw new AppError(
      preview.reason || "Cannot publish: no changes since last report",
      400,
    );
  }
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO ward_published_reports
...
export async function listPublishedReports(wardId: string, page = 1, limit = 10) {
  await ensureWardPublishingSchema();

  const offset = (page - 1) * limit;

  const countRes = await pool.query(
    `SELECT COUNT(*)::int AS total FROM ward_published_reports WHERE ward_id = $1`,
    [wardId],
  );
...
export async function listPublicPublishedReports(page = 1, limit = 10) {
  ...
  const countRes = await pool.query(
    `SELECT COUNT(*)::int AS total FROM ward_published_reports`,
  );
  const total = countRes.rows[0]?.total ?? 0;
  ...
  return {
    reports: rows.map((r) => {
      const snapshot = r.report_snapshot as TaskSnapshot[];
      const planned = snapshot.filter((t) => t.status === "incoming").length;
      const inProgress = snapshot.filter((t) => t.status === "in_progress").length;
      const completed = snapshot.filter((t) => t.status === "completed").length;
      const closed = snapshot.filter((t) => ["invalid", "returned"].includes(t.status)).length;
...
export async function getPublicPublishedReport(reportId: string) {
  const schema = await getWardPublishingSchemaStatus();
  ...
  const { rows } = await pool.query(
    `SELECT wpr.id, wpr.ward_id, wpr.published_at, wpr.cycle_start, wpr.cycle_end,
  ...
  const report = rows[0];
  const snapshot = report.report_snapshot as TaskSnapshot[];
  const prevSnapshot = report.previous_snapshot as TaskSnapshot[] | null;
