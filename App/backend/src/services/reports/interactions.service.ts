...
async function syncReportUpvoteCount(reportId: string) {
  const { rows } = await pool.query<{ upvote_count: number }>(
...
// ─── Report Detail ───────────────────────────────────────────────────
export async function getReportDetailService(
  reportId: string, // ✅ UUID string
  actorId?: string | null, // ✅ UUID string
  ...
  const params: any[] = actorId ? [reportId, actorId] : [reportId];
...
// ─── Upvote (toggle) ─────────────────────────────────────────────────
export async function toggleUpvoteService(
  reportId: string, // ✅ UUID string
  ...
  const report = await pool.query(
    `SELECT id, user_id, is_public FROM reports WHERE id = $1`,
    [reportId],
  );
  ...
  const existing = await pool.query(
    `SELECT id FROM upvotes WHERE report_id = $1 AND ${actorColumn} = $2`,
    [reportId, actorId],
  );

  if (existing.rows.length) {
    await pool.query(
      `DELETE FROM upvotes WHERE report_id = $1 AND ${actorColumn} = $2`,
      [reportId, actorId],
    );
...
export async function getCommentsService(
  ...
  const countResult = await pool.query(
    `SELECT COUNT(*)::int AS total FROM comments WHERE report_id = $1`,
    [reportId],
  );
...
export async function addCommentService({
  ...
  const report = await pool.query(
    `SELECT id, is_public FROM reports WHERE id = $1`,
    [reportId],
  );
...
export async function getReportChangelogService(reportId: string): Promise<ChangelogEvent[]> {
  const { rows: rr } = await pool.query(
    `SELECT r.submitted_at, r.created_at,
  ...
  if (!rr.length) throw new Error("Report not found");
  const r = rr[0];
...
export async function toggleBookmarkService(
  ...
  const existing = await pool.query(
    `SELECT id FROM bookmarks WHERE report_id = $1 AND user_id = $2`,
    [reportId, userId],
  );

  if (existing.rows.length) {
    await pool.query(
      `DELETE FROM bookmarks WHERE report_id = $1 AND user_id = $2`,
      [reportId, userId],
    );
  ...
  await pool.query(
    `INSERT INTO bookmarks (report_id, user_id) VALUES ($1, $2)`,
    [reportId, userId],
  );
