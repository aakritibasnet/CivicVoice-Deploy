...
type CreateReportInput = {
  ...
  locationAccuracyM?: string | number | null;
  address?: string | null;
...
async function findInitialKanbanColumn(
  client: PoolClient,
  role: "ward" | "admin",
) {
...
export async function createReportService(input: CreateReportInput) {
  ...
  const verifiedPriority = verifyPrioritySuggestionToken(
    aiPriorityToken,
    fileBuffer,
  );
  ...
  try {
    ...
    const initialColumn = await findInitialKanbanColumn(
      client,
      wardId ? "ward" : "admin",
    );
    const now = new Date();
    ...
    if (userId === null && deviceId) {
      await client.query(
        `INSERT INTO anonymous_reports (report_id, device_id) VALUES ($1, $2)`,
        [report.id, deviceId]
      );
    }
...
// ─── List My Reports ─────────────────────────────────────────────────
export async function listMyReportsService(params: { userId: string }) {
  const { userId } = params;
...
function timeRangeToInterval(range: PublicReportsParams["timeRange"]): string | null {
  switch (range) {
...
function normalizeCategoryFilter(value: PublicReportsParams["category"]): string[] {
  if (!value) return [];
...
export async function findSimilarReports(
  ...
  const params: any[] = [lat, lng, category, radiusM, limitRows];
...
export async function listPublicReportsService(
  params: PublicReportsParams & {
    status?: string | null;
  ...
  if (
    ...
    conditions.push(
      `r.location_lat IS NOT NULL AND r.location_lng IS NOT NULL AND
       (6371000 * acos(least(1, greatest(-1,
...
type ClaimResult = {
  claimed: string[];
  errors: { reportId: string; reason: string }[];
};
...
export async function claimReportsService(params: {
  ...
  const errors: { reportId: string; reason: string }[] = [];
  ...
  try {
    ...
    for (const reportId of reportIds) {
      ...
      const { rows: anonRows } = await client.query(
        `SELECT 1 FROM anonymous_reports WHERE report_id = $1 AND device_id = $2`,
        [reportId, deviceId]
      );
      ...
      await client.query(
        `UPDATE reports SET user_id = $1, updated_at = NOW() WHERE id = $2`, 
        [userId, reportId]
      );

      // 4) Move to audit trail
      await client.query(
        `INSERT INTO anonymous_report_claims (report_id, device_id, claimed_by) 
         VALUES ($1, $2, $3)`,
        [reportId, deviceId, userId]
      );

      // 5) Remove from junction table
      await client.query(
        `DELETE FROM anonymous_reports WHERE report_id = $1`, 
        [reportId]
      );
...
export async function getAnonymousReportsByDeviceService(deviceId: string) {
  const q = `
    SELECT
      r.id,
      r.title,
