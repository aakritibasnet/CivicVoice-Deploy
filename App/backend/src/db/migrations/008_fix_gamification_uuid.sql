-- Migration 008: Fix gamification functions for UUID user/badge IDs
-- Tables (user_stats, user_badges, badges) already exist correctly from Prisma with UUID types.
-- This migration only recreates the PL/pgSQL functions and trigger that were missing or had wrong types.

-- 1) Drop old trigger and functions (safe even if they don't exist)
...
-- 2) Fix indexes on reports to use user_id
...
-- 3) refresh_user_stats(UUID) — computes stats from reports table
...
BEGIN
  ...
  ELSE
    WITH days AS (
      SELECT DISTINCT created_at::DATE AS d FROM reports WHERE user_id = p_user_id
    ),
    ordered AS (
      SELECT d, ROW_NUMBER() OVER (ORDER BY d DESC) AS rn FROM days
    ),
    grouped AS (
      SELECT d FROM ordered WHERE (SELECT MAX(d) FROM days) - d = rn - 1
    )
  ...
  INSERT INTO user_stats (
    user_id, total_reports, resolved_reports, total_upvotes_received,
    current_streak_days, longest_streak_days, last_report_date, impact_score, updated_at
  )
  VALUES (
    p_user_id, v_total_reports, v_resolved_reports, v_total_upvotes,
    COALESCE(v_current_streak, 0),
    COALESCE(GREATEST(v_longest_streak, COALESCE(v_current_streak, 0)), 0),
    v_last_date, v_stats.impact_score, NOW()
  )
...
-- 4) Trigger to keep user_stats in sync
...
-- 5) check_and_award_badges(UUID) — returns UUID[] since badges.id is UUID
CREATE OR REPLACE FUNCTION check_and_award_badges(p_user_id UUID)
RETURNS UUID[] AS $$
DECLARE
  v_stats user_stats;
  v_badge RECORD;
  v_new_badge_ids UUID[] := '{}';
