...
function reportScoreExpr(scope, placeholder) {
    if (scope === "place") {
        return `
...
export async function searchReports(params) {
    const { query, scope = "general", category, status, startDate, endDate, bounds, page, limit, } = params;
    ...
    const whereClauses = ["r.is_public = TRUE"];
    const values = [];
    let idx = 1;
    const searchableFields = reportSearchFields(scope);
    for (const term of terms) {
        const like = `%${term}%`;
        whereClauses.push(`(${searchableFields.map((field) => `${field} ILIKE $${idx}`).join(" OR ")})`);
        values.push(like);
        idx += 1;
    }
    if (category) {
    ...
    if (bounds) {
        whereClauses.push(`r.location_lat BETWEEN $${idx} AND $${idx + 1} AND r.location_lng BETWEEN $${idx + 2} AND $${idx + 3}`);
...
export async function searchDirectory(query, limit) {
    const trimmed = query.trim();
    ...
    if (hasQuery) {
        ...
        officerWhere.push(`(u.name ILIKE $1 OR COALESCE(d.name, '') ILIKE $1 OR COALESCE(w.name, '') ILIKE $1 OR COALESCE(w.ward_code, '') ILIKE $1)`);
    }
  ...
  `;
    ...
    if (hasQuery) {
        ...
        departmentWhere.push(`(d.name ILIKE $1 OR COALESCE(d.description, '') ILIKE $1 OR COALESCE(w.name, '') ILIKE $1 OR COALESCE(w.ward_code, '') ILIKE $1)`);
    }
    departmentsValues.push(limit);
    const departmentsLimitPlaceholder = `$${departmentsValues.length}`;
    const departmentsWhereClause = departmentWhere.length > 0 ? `WHERE ${departmentWhere.join(" AND ")}` : "";
  ...
  `;
    const [officersRes, departmentsRes] = await Promise.all([
        pool.query(officersSql, officerValues),
        pool.query(departmentsSql, departmentsValues),
    ]);
    return {
...
export async function getPublicOfficerDetail(officerId) {
