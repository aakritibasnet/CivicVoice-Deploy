...
import { uploadBufferToCloudinary, deleteFromCloudinary, } from "@/lib/upload.cloudinary";
/** Inputs */
...
async function blockIfWardAccount(userId) {
    const res = await pool.query(`SELECT role, deleted_at FROM users WHERE id = $1`, [userId]);
...
/** Edit full name */
export async function editFullNameService(jwtUser, body) {
...
export async function updateProfileImageService(jwtUser, file) {
    const userId = mustUserId(jwtUser);
    if (!file)
        throw new AppError("Image file is required", 400);
    // Delete old image from Cloudinary if one exists
    const existing = await pool.query(`SELECT profile_image_public_id FROM users WHERE id = $1`, [userId]);
    ...
    await pool.query(`UPDATE users SET profile_image_url = $1, profile_image_public_id = $2 WHERE id = $3`, [result.secure_url, result.public_id, userId]);
 ...
 */
export async function requestEmailChangeService(jwtUser, body) {
    const userId = mustUserId(jwtUser);
    ...
    const pending = await pool.query(`SELECT id FROM pending_users WHERE email = $1`, [newEmail]);
    if (pending.rows.length > 0)
        throw new AppError("Email already pending verification.", 409);
    const currentUser = await pool.query(`SELECT email FROM users WHERE id = $1 AND deleted_at IS NULL`, [userId]);
 ...
 */
export async function confirmEmailChangeService(jwtUser, body) {
    const userId = mustUserId(jwtUser);
    ...
    const verifiedMeta = verified.meta && typeof verified.meta === "object" && !Array.isArray(verified.meta)
        ? verified.meta
        : {};
    ...
    if (metaUserId !== userId || metaEmail !== newEmail) {
        throw new AppError("Verification mismatch. Please request a new code.", 400);
    }
    const existing = await pool.query(`SELECT id FROM users WHERE email = $1`, [
        newEmail,
    ]);
 ...
 */
export async function getPublicProfileService(userId) {
    const userRes = await pool.query(`SELECT id, name, profile_image_url, created_at
     FROM users
    ...
    try {
        const statsRes = await pool.query(`SELECT total_reports, resolved_reports, current_streak_days, longest_streak_days, impact_score
    ...
    let badges = [];
    try {
        const badgesRes = await pool.query(`SELECT b.id, b.name, b.description, b.icon_name, b.tier, ub.earned_at
       FROM user_badges ub
...
export async function changePasswordService(jwtUser, body) {
    const userId = mustUserId(jwtUser);
    await blockIfWardAccount(userId);
    ...
    const userRes = await pool.query(`SELECT password_hash FROM users WHERE id = $1 AND deleted_at IS NULL`, [userId]);
    if (userRes.rows.length === 0)
        throw new AppError("User not found", 404);
