...
export async function deleteAccountService(jwtUser, body) {
    const userId = mustUserId(jwtUser);
    // Ward organizational accounts cannot be deleted via API
    const wardCheck = await pool.query(`SELECT role, deleted_at FROM users WHERE id = $1`, [userId]);
    if (wardCheck.rows.length === 0 || wardCheck.rows[0].deleted_at) {
        throw new AppError("User not found", 404);
    }
    if (["ward", "municipality", "admin"].includes(wardCheck.rows[0].role)) {
        throw new AppError("Ward organizational accounts cannot be deleted from the app. Contact database administrator.", 403);
    }
    ...
    try {
        ...
        // already deleted?
        const check = await client.query(`SELECT id, deleted_at FROM users WHERE id = $1`, [userId]);
