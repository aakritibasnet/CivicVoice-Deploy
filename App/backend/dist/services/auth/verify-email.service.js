...
export async function verifyEmailService(body) {
    ...
    if (!otp) {
        throw new AppError("No verification code found. Please sign up again.", 404);
    }
    if (!otp.target) {
        throw new AppError("Verification data is missing. Please request a new verification code.", 400);
    }
    if (normalizeEmail(otp.target) !== normalizedEmail) {
        throw new AppError("Verification target mismatch. Please request a new verification code.", 400);
    }
    if (otp.used_at) {
