...
import { notifyNewComment, notifyStatusChange, } from "@/services/notifications/triggers.service";
// Map mobile status names to DB enum values
// Mobile "todo" → DB "incoming"
function toDbStatus(mobileStatus) {
    if (mobileStatus === "todo")
        return "incoming";
    return mobileStatus; // in_progress, completed, invalid are the same
}
function toMobileStatus(dbStatus) {
    if (dbStatus === "incoming")
        return "todo";
    if (dbStatus === "returned")
        return "todo"; // treat returned as todo
    return dbStatus;
}
async function logOfficerActivity(reportId, officerId, action, details = {}) {
    await pool.query(`INSERT INTO activity_log (report_id, actor_id, actor_name, action, details)
     VALUES (
       $1,
       NULL,
...
async function findWorkflowKanbanColumnId(mappedStatus, assignedLevel) {
    if (mappedStatus !== "incoming" &&
    ...
    const res = await pool.query(`SELECT id
     ...
     ORDER BY
       ...
       position ASC,
       created_at ASC
     LIMIT 1`, [mappedStatus, workflowRole]);
    return res.rows[0]?.id ?? null;
}
async function resolveOfficerAccessScope(officerId) {
    const officerRes = await pool.query(`SELECT o.type,
            o.ward_id,
            w.name AS ward_name,
            w.municipality_id,
     ...
     WHERE o.id::text = $1
       AND o.deleted_at IS NULL`, [officerId]);
    if (officerRes.rows.length > 0) {
        ...
        let scopeSource = municipalityId
            ? "ward"
            : "none";
        if (row.type === "municipality_officer" && !municipalityId) {
            const fallbackRes = await pool.query(`SELECT w.municipality_id,
                m.name AS municipality_name
         FROM reports r
    ...
    const userRes = await pool.query(`SELECT u.ward_id,
            u.municipality_id,
            w.name AS ward_name,
...
export async function getOfficerTasks(officerId, filters) {
    ...
    else if (scope.officerType === "municipality_officer" &&
    ...
    if (filters.escalated_only) {
        conditions.push(`(r.assigned_level = 'municipality' OR r.escalated_to_municipality = true)`);
    }
    const where = conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "";
    const res = await pool.query(`SELECT r.id, r.title, r.description, r.category, r.subcategory,
...
export async function getTaskDetail(taskId, officerId) {
    const taskRes = await pool.query(`SELECT r.id, r.title, r.description, r.category, r.subcategory,
            r.address_text AS location_text,
            r.location_lat, r.location_lng,
            r.ward_id, r.assigned_department_id AS department_id,
            r.assigned_field_officer_id AS assigned_officer_id,
    ...
    // Activity timeline
    const activityRes = await pool.query(`SELECT al.id, al.report_id AS task_id, al.actor_id, al.actor_name,
     ...
     ORDER BY al.created_at DESC`, [taskId]);
    ...
    // Comments
    const commentsRes = await pool.query(`SELECT c.id, c.report_id AS task_id,
    ...
    const proofRes = await pool.query(`SELECT tc.id, tc.task_id, tc.completed_by_officer_id AS officer_id,
            tc.description AS note,
            tc.before_image_url, tc.after_image_url AS image_url,
            'completion' AS type,
            tc.created_at
     FROM task_completions tc
     WHERE tc.task_id = $1
     ORDER BY tc.created_at DESC`, [taskId]);
    return {
        task,
        activity,
...
export async function updateTaskStatus(taskId, officerId, newStatus, note) {
    // Verify the report exists and is assigned to this officer
    const taskRes = await pool.query(`SELECT id, status, assigned_field_officer_id, assigned_level
     FROM reports
     WHERE id = $1`, [taskId]);
    if (taskRes.rows.length === 0) {
        throw new AppError("Task not found", 404);
    ...
    if (!validTransitions[oldDbStatus]?.includes(newDbStatus)) {
        throw new AppError(`Cannot move task from '${oldMobileStatus}' to '${newStatus}'`, 400);
    }
    // If completing, check task_completions proof exists
    if (newDbStatus === "completed") {
        const proofRes = await pool.query(`SELECT COUNT(*)::int AS count FROM task_completions WHERE task_id = $1`, [taskId]);
        if (proofRes.rows[0].count === 0) {
            throw new AppError("Cannot complete task without uploading proof. Please upload at least 1 proof image.", 400);
        }
    }
    // Update report status
    const updateFields = [`status = $2::report_status`];
    const updateParams = [taskId, newDbStatus];
    let paramIdx = 3;
    const targetKanbanColumnId = await findWorkflowKanbanColumnId(newDbStatus, task.assigned_level);
    ...
    await pool.query(`UPDATE reports SET ${updateFields.join(", ")} WHERE id = $1`, updateParams);
    // Log activity
...
// ─── Proof (= task_completions) ───────────────────────────────────────
export async function uploadTaskProof(taskId, officerId, imageUrl, type = "completion", note) {
    // Verify report exists and is assigned to officer
    const taskRes = await pool.query(`SELECT id, status, assigned_field_officer_id FROM reports WHERE id = $1`, [taskId]);
    if (taskRes.rows.length === 0) {
        throw new AppError("Task not found", 404);
    }
    const task = taskRes.rows[0];
    ...
    const existingRes = await pool.query(`SELECT id FROM task_completions WHERE task_id = $1`, [taskId]);
    let proofRow;
    if (existingRes.rows.length > 0) {
        ...
        const updateRes = await pool.query(`UPDATE task_completions
       SET ${updateField} = $2, description = $3, completed_by_officer_id = $4, updated_at = NOW()
       WHERE task_id = $1
       RETURNING *`, [taskId, imageUrl, note.trim(), officerId]);
        ...
        const res = await pool.query(`INSERT INTO task_completions (task_id, completed_by_officer_id, description, ${imgField}, ${otherField})
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [taskId, officerId, note.trim(), imageUrl, type === "invalidation" ? imageUrl : null]);
        proofRow = res.rows[0];
    }
    // Log activity
...
// ─── Reports ───────────────────────────────────────────────────────────
export async function getOfficerReports(officerId, wardId) {
    const scope = await resolveOfficerAccessScope(officerId);
    const conditions = [];
    const params = [];
    let idx = 1;
    ...
    const res = await pool.query(`SELECT r.id, r.title, r.description, r.category,
     ...
     LEFT JOIN municipalities m ON m.id::text = w.municipality_id::text
     ${where}
     ORDER BY r.submitted_at DESC, r.created_at DESC`, params);
    return { reports: res.rows };
}
export async function getOfficerReportDetail(reportId, officerId) {
    const res = await pool.query(`SELECT r.id, r.title, r.description, r.category,
            r.status, r.media_url, r.photo_urls, r.address_text,
            r.location_lat, r.location_lng, r.submitted_at, r.created_at,
     ...
     LEFT JOIN wards w ON w.id::text = r.ward_id::text
     LEFT JOIN users u ON u.id::text = r.user_id::text
     WHERE r.id::text = $1`, [reportId]);
    if (res.rows.length === 0) {
        throw new AppError("Report not found", 404);
    }
    // Get comments
    const commentsRes = await pool.query(`SELECT c.id, c.user_id,
            c.officer_id,
            c.public_tag,
            c.content, c.created_at,
            COALESCE(c.public_tag, u.name, o.first_name || ' ' || o.last_name) AS author_name,
...
export async function addReportComment(reportId, officerId, content, publicTag) {
    const res = await pool.query(`INSERT INTO comments (report_id, user_id, officer_id, public_tag, content)
     VALUES ($1, NULL, $2, $3, $4)
     RETURNING *`, [reportId, officerId, publicTag, content]);
    // Increment comment count
    await pool.query(`UPDATE reports SET comment_count = comment_count + 1 WHERE id = $1`, [reportId]);
    await notifyNewComment(reportId, officerId, content);
    return { comment: { ...res.rows[0], public_tag: publicTag }, publicTag };
}
// ─── Task Comments ─────────────────────────────────────────────────────
export async function addTaskComment(taskId, officerId, content, publicTag) {
    // Tasks are reports — comments go into the same comments table
    const res = await pool.query(`INSERT INTO comments (report_id, user_id, officer_id, public_tag, content)
     VALUES ($1, NULL, $2, $3, $4)
     RETURNING *`, [taskId, officerId, publicTag, content]);
    // Increment comment count
    await pool.query(`UPDATE reports SET comment_count = comment_count + 1 WHERE id = $1`, [taskId]);
    // Log activity
...
// ─── Notifications ─────────────────────────────────────────────────────
export async function getOfficerNotifications(officerId, unreadOnly) {
    const conditions = [`COALESCE(n.officer_id::text, n.user_id::text) = $1`];
    if (unreadOnly)
        conditions.push(`n.is_read = false`);
    const res = await pool.query(`SELECT n.id,
...
export async function getOfficerUnreadCount(officerId) {
    const res = await pool.query(`SELECT COUNT(*)::int AS count FROM notifications
     WHERE COALESCE(officer_id::text, user_id::text) = $1
       AND is_read = false`, [officerId]);
    return { count: res.rows[0].count };
}
export async function markOfficerNotificationRead(notifId, officerId) {
    await pool.query(`UPDATE notifications SET is_read = true
     WHERE id = $1
       AND COALESCE(officer_id::text, user_id::text) = $2`, [notifId, officerId]);
    return { success: true };
}
export async function markAllOfficerNotificationsRead(officerId) {
    const res = await pool.query(`UPDATE notifications SET is_read = true
     WHERE COALESCE(officer_id::text, user_id::text) = $1
       AND is_read = false`, [officerId]);
    return { updated: res.rowCount };
}
// ─── History ───────────────────────────────────────────────────────────
export async function getOfficerHistory(officerId, type) {
    const conditions = [`r.assigned_field_officer_id = $1`];
    if (type === "completed") {
        conditions.push(`r.status = 'completed'`);
    }
    const res = await pool.query(`SELECT r.id, r.title, r.category, r.address_text AS location_text,
            r.status, r.priority,
            r.submitted_at AS assigned_at,
            r.actual_completion_date AS completed_at,
            w.name AS ward_name,
     ...
     LEFT JOIN officer_departments od ON od.id::text = r.assigned_department_id::text
     WHERE ${conditions.join(" AND ")}
     ORDER BY COALESCE(r.actual_completion_date, r.submitted_at) DESC`, [officerId]);
    // Map status
...
export async function getOfficerProfile(officerId) {
    ...
    const officerRes = await pool.query(`SELECT o.id, o.first_name, o.last_name, o.email, o.profile_image_url,
     ...
     WHERE o.id = $1 AND o.deleted_at IS NULL`, [officerId]);
    if (officerRes.rows.length > 0) {
        const row = officerRes.rows[0];
        if (row.type === "municipality_officer" && scope.municipalityId) {
            const statsRes = await pool.query(`SELECT COUNT(*)::int AS total_tasks,
         ...
         FROM reports r
         LEFT JOIN wards w ON w.id::text = r.ward_id::text
         WHERE w.municipality_id::text = $1`, [scope.municipalityId]);
    ...
    const userRes = await pool.query(`SELECT u.id, u.name, u.email, u.role, u.profile_image_url,
     ...
     FROM users u
     WHERE u.id = $1 AND u.role = 'officer'`, [officerId]);
    if (userRes.rows.length === 0) {
        throw new AppError("Officer profile not found", 404);
    }
    return { profile: userRes.rows[0] };
}
export async function updateOfficerPhoto(officerId, imageUrl) {
    // Try officers table first
    const officerRes = await pool.query(`UPDATE officers SET profile_image_url = $2, updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id`, [officerId, imageUrl]);
    if (officerRes.rows.length === 0) {
        // Fallback to users table
        await pool.query(`UPDATE users SET profile_image_url = $2 WHERE id = $1`, [officerId, imageUrl]);
    }
...
export async function changeOfficerPassword(officerId, oldPassword, newPassword) {
    ...
    let table = "officers";
    if (res.rows.length === 0) {
        res = await pool.query(`SELECT password_hash FROM users WHERE id = $1`, [officerId]);
        table = "users";
    }
    if (res.rows.length === 0) {
    ...
    if (table === "officers") {
        await pool.query(`UPDATE officers SET password_hash = $2, must_change_password = false, password_changed_at = NOW() WHERE id = $1`, [officerId, hash]);
    }
    else {
        await pool.query(`UPDATE users SET password_hash = $2 WHERE id = $1`, [officerId, hash]);
    }
    return { success: true, message: "Password changed successfully" };
}
// ─── Build Public Tag ──────────────────────────────────────────────────
export async function getOfficerPublicTag(officerId) {
    // Try officers table first
    const officerRes = await pool.query(`SELECT o.type, o.ward_id, w.name AS ward_name
     FROM officers o
     LEFT JOIN wards w ON w.id = o.ward_id
     WHERE o.id = $1 AND o.deleted_at IS NULL`, [officerId]);
    if (officerRes.rows.length > 0) {
    ...
    // Fallback to users table
    const userRes = await pool.query(`SELECT u.role, u.ward_id, w.name AS ward_name
     FROM users u
     LEFT JOIN wards w ON w.id = u.ward_id
     WHERE u.id = $1`, [officerId]);
    if (userRes.rows.length === 0)
        return "Officer";
    const row = userRes.rows[0];
    if (row.ward_id && row.ward_name) {
