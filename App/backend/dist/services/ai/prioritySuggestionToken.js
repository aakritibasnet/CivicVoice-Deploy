...
export function verifyPrioritySuggestionToken(token, imageBuffer) {
    const secret = getSigningSecret();
    if (!token || !secret) {
        return null;
    }
    const [version, priority, imageHash, signature] = token.split(".");
    if (version !== AI_PRIORITY_TOKEN_VERSION ||
