...
function getOfficerTaskLink(reportId) {
    return `/officer-task/${reportId}`;
}
async function getCommentAuthorName(commenterId) {
    const result = await pool.query(`SELECT COALESCE(u.name, o.first_name || ' ' || o.last_name, 'Someone') AS name
     FROM (SELECT $1::uuid AS id) ref
     LEFT JOIN users u ON u.id = ref.id
     LEFT JOIN officers o ON o.id = ref.id`, [commenterId]);
    return result.rows[0]?.name || "Someone";
}
export async function notifyStatusChange(reportId, newStatus) {
    const { rows } = await pool.query(`SELECT user_id, title, assigned_field_officer_id
     FROM reports
     WHERE id = $1`, [reportId]);
    const row = rows[0];
    if (!row)
    ...
    try {
        ...
        await Promise.all(followersToNotify.map((followerId) => createNotification({
            userId: followerId,
            reportId,
            type: "status_change",
    ...
    let officerType = "task_status_updated";
    switch (newStatus) {
        case "returned":
...
export async function notifyNewComment(reportId, commenterId, content) {
    const reportRes = await pool.query(`SELECT user_id, title, assigned_field_officer_id
     FROM reports
     WHERE id = $1`, [reportId]);
    ...
    if (report.assigned_field_officer_id &&
...
export async function notifyReportPostComment(postId, commenterId, content) {
    const postRes = await pool.query(`SELECT
       rp.title,
       rp.source_user_id,
    ...
    const sent = new Set();
    const notify = async (recipientId, recipientRole, title, message) => {
    ...
    await notify(post.source_user_id, "citizen", "New comment on an update for your report", `${commenterName} commented on "${postTitle}": ${preview}`);
    await notify(post.report_owner_id, "citizen", "New comment on an update for your report", `${commenterName} commented on "${postTitle}": ${preview}`);
    await notify(post.completed_by_user_id, "citizen", "New comment on your completion post", `${commenterName} commented on "${postTitle}": ${preview}`);
    await notify(post.completed_by_officer_id, "officer", "New comment on your completion post", `${commenterName} commented on "${postTitle}": ${preview}`);
}
// ─── Comment Reply ──────────────────────────────────────────────────
export async function notifyCommentReply(reportId, parentCommentId, replierId, content) {
    // Find the parent comment author
    const parentRes = await pool.query(`SELECT user_id, officer_id FROM comments WHERE id = $1`, [parentCommentId]);
    ...
    const preview = content.length > 80 ? `${content.slice(0, 77)}...` : content;
    const reportRes = await pool.query(`SELECT title FROM reports WHERE id = $1`, [reportId]);
...
export async function notifyReportPostReply(postId, parentCommentId, replierId, content) {
    const parentRes = await pool.query(`SELECT user_id FROM report_comments WHERE id = $1`, [parentCommentId]);
    const parent = parentRes.rows[0];
    if (!parent?.user_id || parent.user_id === replierId)
        return;
    const replierName = await getCommentAuthorName(replierId);
    const preview = content.length > 80 ? `${content.slice(0, 77)}...` : content;
    const postRes = await pool.query(`SELECT title FROM report_posts WHERE id = $1`, [postId]);
...
export async function notifyUpvoteMilestone(reportId, newCount) {
    if (!UPVOTE_MILESTONES.includes(newCount))
        return;
    const { rows } = await pool.query(`SELECT user_id, title
     FROM reports
     WHERE id = $1`, [reportId]);
    const report = rows[0];
    if (!report || report.user_id == null)
        return;
...
export async function notifyNearbyResolved(params) {
    const { rows } = await pool.query(`SELECT id, user_id, location_lat, location_lng
     FROM reports
     WHERE id = $1 AND status = 'completed'`, [params.reportId]);
    ...
    const usersRes = await pool.query(`SELECT DISTINCT r.user_id
    FROM reports r
    ...
    const message = "An issue near one of your recent reports has been resolved. Your reports are helping improve your area!";
    await Promise.all(usersRes.rows
        .filter((row) => row.user_id !== ownerUserId)
        .map((row) => createNotification({
        userId: row.user_id,
...
export async function notifyLeaderboardRank(userId) {
    ...
    const { rows } = await pool.query(`WITH ranked AS (
     ...
     SELECT rank::int, total_users::int FROM ranked WHERE id = $1`, [userId]);
    const row = rows[0];
    if (!row)
        return;
    const rank = row.rank;
    if (!LEADERBOARD_RANK_MILESTONES.includes(rank))
        return;
    // Check if we already sent this exact rank notification recently (last 7 days)
    const dupeCheck = await pool.query(`SELECT 1 FROM notifications
     WHERE user_id = $1
       AND type = 'leaderboard_rank'
       AND metadata->>'rank' = $2
       AND created_at >= NOW() - INTERVAL '7 days'
     LIMIT 1`, [userId, String(rank)]);
...
export async function notifyWardUsersOfNewReport(reportId) {
    const { rows } = await pool.query(`SELECT title, ward_id
     FROM reports
     WHERE id = $1`, [reportId]);
    const report = rows[0];
    if (!report?.ward_id)
        return;
    const wardUsers = await pool.query(`SELECT id
     FROM users
     WHERE role = 'ward'
       AND ward_id = $1
       AND is_active = TRUE
       AND deleted_at IS NULL`, [report.ward_id]);
    if (wardUsers.rows.length === 0)
        return;
    const title = "New task assigned to your ward";
    const message = `A new task "${report.title || "Untitled"}" has been assigned to your ward.`;
    await Promise.all(wardUsers.rows.map((wardUser) => createNotification({
        userId: wardUser.id,
        reportId,
