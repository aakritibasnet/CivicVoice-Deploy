...
export async function setMute(principal, chatId, duration) {
    const valid = ["15m", "1h", "tomorrow", "forever", "off"];
    ...
    const { rows } = await pool.query(`UPDATE chat_participants
        SET muted_until = ${expr}
      WHERE chat_id = $1 AND party_kind = $2 AND party_id = $3
    RETURNING muted_until`, [chatId, principal.kind, principal.id]);
    if (!rows[0])
