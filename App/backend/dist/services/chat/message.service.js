...
async function findByClientMsgId(chatId, clientMsgId) {
    const { rows } = await pool.query(`SELECT ${SELECT_COLS} FROM messages
      WHERE chat_id = $1 AND client_msg_id = $2`, [chatId, clientMsgId]);
    return rows[0] ?? null;
}
export async function sendMessage(principal, chatId, input) {
    ...
    if (!allowed) {
        throw new AppError("Sending is restricted to admins in this group", 403);
    }
    ...
    try {
        ...
        try {
         ...
         RETURNING ${SELECT_COLS}`, [
            ...
            if (err?.code === "23505" && input.clientMsgId) {
        ...
        const mentions = (input.mentions ?? []).filter((m) => m && (m.kind === "user" || m.kind === "officer") && m.id);
        for (const m of mentions) {
            await client.query(`INSERT INTO chat_mentions (message_id, mentioned_kind, mentioned_id)
         VALUES ($1, $2::text, $3::uuid)
         ON CONFLICT (message_id, mentioned_kind, mentioned_id) DO NOTHING`, [row.id, m.kind, m.id]);
        }
        await client.query(`UPDATE chats
          SET last_message_at = $2, updated_at = CURRENT_TIMESTAMP
        WHERE id = $1`, [chatId, row.created_at]);
...
function cursorOf(row) {
    return Buffer.from(`${row.created_us}|${row.id}`, "utf8").toString("base64url");
}
...
export async function getHistory(principal, chatId, opts) {
    ...
    if (after) {
        const { rows } = await pool.query(`SELECT ${HISTORY_COLS} FROM messages m
        WHERE m.chat_id = $1
          AND ((extract(epoch from m.created_at) * 1000000)::bigint, m.id)
              > ($2::bigint, $3::uuid)
        ORDER BY m.created_at ASC, m.id ASC
        LIMIT $4`, [chatId, after.createdUs, after.id, limit + 1]);
        const hasMore = rows.length > limit;
        const page = rows.slice(0, limit);
        return {
            messages: page,
            nextCursor: page.length > 0 ? cursorOf(page[page.length - 1]) : opts.after ?? null,
            hasMore,
            direction: "forward",
        };
    }
    ...
    const { rows } = await pool.query(`SELECT ${HISTORY_COLS} FROM messages m
      ...
      LIMIT $4`, [chatId, before?.createdUs ?? null, before?.id ?? null, limit + 1]);
    const hasMore = rows.length > limit;
    const page = rows.slice(0, limit);
    return {
        messages: page,
