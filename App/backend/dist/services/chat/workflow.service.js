...
export async function resolveComplaint(complaintId) {
    const { rows } = await pool.query(`SELECT id, ward_id FROM reports WHERE id = $1`, [complaintId]);
    if (!rows[0])
        throw new AppError("Linked complaint not found", 404);
    return { wardId: rows[0].ward_id ?? null };
}
async function notifyParticipants(chatId, exclude, type, title, message) {
    const { rows } = await pool.query(`SELECT party_kind, party_id FROM chat_participants
      WHERE chat_id = $1 AND is_active = TRUE
        AND NOT (party_kind = $2 AND party_id = $3)`, [chatId, exclude.kind, exclude.id]);
    await Promise.allSettled(rows.map((r) => createNotification({
        userId: r.party_id,
...
export async function escalateChat(principal, chatId, input) {
    ...
    try {
        ...
        const res = await client.query(`UPDATE chats
      ...
      RETURNING id, status::text AS status, priority::text AS priority,
                department_id`, [chatId, input.priority ?? null, input.departmentId ?? null]);
        ...
        await notifyParticipants(chatId, principal, "chat_escalated", "Chat escalated", input.reason);
...
export async function setChatStatus(principal, chatId, next) {
    ...
    try {
        await client.query("BEGIN");
        const cur = await client.query(`SELECT status::text AS status FROM chats WHERE id = $1 FOR UPDATE`, [chatId]);
        if (!cur.rows[0])
            throw new AppError("Chat not found", 404);
        ...
        await client.query(`UPDATE chats SET status = $2::chat_status,
              updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [chatId, next]);
        await writeAudit(client, {
            chatId,
            actor: principal,
        ...
        await client.query("COMMIT");
        if (next === "closed") {
            await notifyParticipants(chatId, principal, "chat_closed", "Chat closed", "This conversation has been closed.");
        }
...
export async function acknowledgeMessage(principal, chatId, messageId) {
    await assertChatAccess(principal, chatId, "workflow");
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        const msg = await client.query(`SELECT id FROM messages WHERE id = $1 AND chat_id = $2`, [messageId, chatId]);
        if (!msg.rows[0])
       ...
       ON CONFLICT (message_id, party_kind, party_id)
         DO UPDATE SET
           ...
           read_at = COALESCE(message_receipts.read_at, EXCLUDED.read_at)`, [messageId, principal.kind, principal.id]);
        await writeAudit(client, {
            chatId,
            actor: principal,
