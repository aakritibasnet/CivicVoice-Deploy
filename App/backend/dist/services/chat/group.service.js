...
export async function addParticipant(principal, chatId, target) {
    await assertChatAccess(principal, chatId, "manage");
    const role = target.role ?? "member";
    const { rowCount } = await pool.query(`INSERT INTO chat_participants (chat_id, party_kind, party_id, role_in_chat)
     ...
     ON CONFLICT (chat_id, party_kind, party_id)
       DO UPDATE SET is_active = TRUE, role_in_chat = EXCLUDED.role_in_chat,
                     removed_at = NULL`, [chatId, target.kind, target.id, role]);
...
export async function removeParticipant(principal, chatId, target) {
    await assertChatAccess(principal, chatId, "manage");
    // Admins cannot remove themselves if they are the only admin.
    if (target.kind === principal.kind && target.id === principal.id) {
        const { rows } = await pool.query(`SELECT COUNT(*)::int AS n FROM chat_participants
        WHERE chat_id = $1 AND role_in_chat = 'admin' AND is_active = TRUE`, [chatId]);
        if (rows[0].n <= 1) {
            throw new AppError("Cannot remove the sole admin", 400);
        }
    }
    await pool.query(`UPDATE chat_participants
        SET is_active = FALSE, removed_at = CURRENT_TIMESTAMP
      WHERE chat_id = $1 AND party_kind = $2 AND party_id = $3`, [chatId, target.kind, target.id]);
    const client = await pool.connect();
    try {
        await writeAudit(client, {
            chatId,
...
export async function updateParticipantRole(principal, chatId, target) {
    await assertChatAccess(principal, chatId, "manage");
    const { rowCount } = await pool.query(`UPDATE chat_participants
        SET role_in_chat = $4::chat_role
      WHERE chat_id = $1 AND party_kind = $2 AND party_id = $3 AND is_active = TRUE`, [chatId, target.kind, target.id, target.role]);
    if (!rowCount)
        throw new AppError("Participant not found or inactive", 404);
    const client = await pool.connect();
    try {
        await writeAudit(client, {
            chatId,
...
export async function renameChat(principal, chatId, title) {
    if (!title?.trim())
        throw new AppError("Title required", 400);
    await assertChatAccess(principal, chatId, "manage");
    await pool.query(`UPDATE chats SET title = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [chatId, title.trim()]);
    const client = await pool.connect();
    try {
        await writeAudit(client, {
            chatId,
...
export async function setRestrictSend(principal, chatId, enabled) {
    await assertChatAccess(principal, chatId, "manage");
    await pool.query(`UPDATE chats SET restrict_send = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [chatId, enabled]);
    const client = await pool.connect();
    try {
        await writeAudit(client, {
            chatId,
...
export async function archiveChat(principal, chatId, archive = true) {
    await assertChatAccess(principal, chatId, "manage");
    await pool.query(`UPDATE chats SET is_archived = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [chatId, archive]);
    const client = await pool.connect();
    try {
        await writeAudit(client, {
            chatId,
 ...
 */
export async function canSendInChat(principal, chatId) {
    const { rows } = await pool.query(`SELECT c.restrict_send,
            cp.role_in_chat
       FROM chats c
      ...
      WHERE c.id = $1`, [chatId, principal.kind, principal.id]);
    if (!rows[0])
        return false;
    if (!rows[0].restrict_send)
        return true;
