...
export async function exportChatHistory(principal, chatId) {
    await assertChatAccess(principal, chatId, "manage");
    const [chatRes, participantsRes, messagesRes, receiptsRes, auditRes] = await Promise.all([
        pool.query(`SELECT id, type::text AS type, title, status::text AS status,
        ...
        pool.query(`SELECT party_kind, party_id, role_in_chat::text, is_active,
                joined_at, removed_at
           FROM chat_participants WHERE chat_id = $1
           ORDER BY joined_at ASC`, [chatId]),
        pool.query(`SELECT id, sender_kind, sender_id, type::text AS type,
        ...
        pool.query(`SELECT mr.message_id, mr.party_kind, mr.party_id,
                mr.delivered_at, mr.read_at, mr.acknowledged_at
           FROM message_receipts mr
           JOIN messages m ON m.id = mr.message_id
          WHERE m.chat_id = $1
          ORDER BY mr.message_id`, [chatId]),
        pool.query(`SELECT id, actor_kind, actor_id, action, metadata, created_at
...
export async function disableParticipant(principal, chatId, target) {
    await assertChatAccess(principal, chatId, "manage");
    const { rowCount } = await pool.query(`UPDATE chat_participants
        SET is_active = FALSE, removed_at = CURRENT_TIMESTAMP
      WHERE chat_id = $1 AND party_kind = $2 AND party_id = $3 AND is_active = TRUE`, [chatId, target.kind, target.id]);
    if (!rowCount)
        throw new AppError("Participant not found or already inactive", 404);
    await writeAudit({ query: pool.query.bind(pool) }, {
        chatId,
        actor: principal,
...
export async function listAllChatsAdmin(principal, opts = {}) {
    ...
    const { rows } = await pool.query(`SELECT c.id, c.type::text AS type, c.title, c.status::text AS status,
