...
import { recordDelivered, recordRead, getUnreadSummary, } from "@/services/chat/receipts.service";
import { acknowledgeMessage } from "@/services/chat/workflow.service";
import { CHAT_NAMESPACE, chatRoom, principalRoom, } from "./io";
import { socketAuthMiddleware } from "./socket-auth";
import { shouldAllowTyping } from "@/middleware/chat-rate-limit";
function principalOf(socket) {
...
export function registerChatNamespace(io) {
    ...
    nsp.on("connection", (socket) => {
        ...
        const inRoom = (chatId) => socket.rooms.has(chatRoom(chatId));
        socket.on("typing:start", (payload) => {
            const chatId = payload?.chatId;
            if (!chatId || !inRoom(chatId))
                return;
            if (!shouldAllowTyping(socket))
        ...
        socket.on("message.read", async (payload, ack) => {
            try {
                ...
                if (!chatId)
                    return ack?.({ ok: false, error: "chatId required" });
                const { unread, lastReadMessageId } = await recordRead(me, chatId, payload?.uptoMessageId ?? null);
        ...
        socket.on("message.acknowledge", async (payload, ack) => {
            try {
        ...
        socket.on("unread:sync", async (_payload, ack) => {
            try {
