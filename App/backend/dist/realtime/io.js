...
export function principalRoom(kind, id) {
...
export function emitToPrincipal(kind, id, event, payload) {
    if (!io)
        return;
...
export async function isPrincipalInChatRoom(kind, id, chatId) {
    if (!io)
        return false;
    const sockets = await io
        ...
        .in(chatRoom(chatId))
        .fetchSockets();
    return sockets.some((s) => s.data?.principal?.kind === kind && s.data?.principal?.id === id);
}
