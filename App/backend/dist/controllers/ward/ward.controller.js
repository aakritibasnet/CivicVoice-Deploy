import { detectWard, detectMunicipality, getWardBoundaries, getMunicipalityBoundaries, } from "../../services/ward/ward.service";
...
export async function listCategoriesController(req, res, next) {
    try {
        const { rows } = await pool.query(`SELECT DISTINCT category FROM reports WHERE category IS NOT NULL ORDER BY category ASC`);
...
export async function detectWardFromCoords(req, res, next) {
    try {
        ...
        if (!ward) {
            ...
            if (municipality) {
                return res.status(200).json({
                    ...
                    message: "Ward boundary not mapped yet, but location is within " +
            ...
            return res.status(200).json({
                ...
                ward: null,
                municipality: null,
                message: "Location is outside all registered boundaries. Your report will still be submitted but may require manual routing.",
            });
        }
        return res.status(200).json({
            success: true,
...
export async function getWardBoundariesController(req, res, next) {
    try {
...
export async function getMunicipalityBoundariesController(req, res, next) {
    try {
        const municipalityId = req.query.municipality_id;
